...
**Next action:** close D8.0 build/runtime readiness first: make `npm run build` pass, verify backend clean start, Docker/Postgres clean start, empty-database migration to Alembic head, and Portal Customer API staging-style configuration. Do not create or refresh proof records, do not enter D9, and do not mark local rehearsal as `STAGING_VALIDATED`.
