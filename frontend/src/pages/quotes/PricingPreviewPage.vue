...
const SAFETY =
  'Pricing preview only. Does not create quotes, send messages, or promise inventory, certification, or lead times.'
...
async function preview() {
  ...
  try {
    ...
    error.value = 'Preview failed. Check backend, FX rate, and product tiers.'
  } finally {
    loading.value = false
  }
}
...
<template>
  <div>
    <h2 class="mb-4 text-lg font-semibold text-slate-800">Pricing Preview</h2>
    ...
    <el-form label-width="140px" class="max-w-xl">
      ...
      <el-form-item>
        <el-button type="primary" :loading="loading" @click="preview">Preview</el-button>
      </el-form-item>
    </el-form>
    <el-alert v-if="error" type="error" :closable="false" show-icon class="mb-3" :title="error" />
    <el-card v-if="result" shadow="never">
      <p class="mb-2 text-sm"><strong>Source:</strong> {{ result.source }}</p>
      <p class="mb-2 text-sm"><strong>Line subtotal:</strong> {{ result.price_breakdown?.line_subtotal }} {{ result.currency }}</p>
      <p class="mb-2 text-sm"><strong>Est. margin:</strong> {{ result.profit_breakdown?.estimated_margin }}%</p>
