...
The JSON record stores check labels, pass/fail states, sanitized URLs, and safety metadata. For a remote staging backend, `backend_base_url` is saved as `https://<redacted-backend>` instead of the real host. Local rehearsal URLs may appear only in scratch outputs kept outside `docs/records`. It does not store the portal token or response bodies.
