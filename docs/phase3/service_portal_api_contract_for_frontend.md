...
The internal `/portal-customer-bridge` UAT page provides a read-only service Portal read path check: manifest, products, orders, then the first returned order's detail, snapshot, production, shipment, and resources. It also renders the snapshot `portal_display` and `customer_timeline` blocks as a service Portal preview so the integration team can verify customer tracking copy and event shape without reading raw JSON. TEST feedback creation remains a separate explicit action.
...
`portal_contract.connection_guide` is the handoff map for `service.intelli-opus.com`: required staging environment readiness, auth header metadata, allowed origins, and the smoke sequence for manifest, products, orders, snapshot, production, shipment, resources, and optional TEST feedback. All read steps are marked `mutates_data=false`; the feedback step is explicitly `test_only=true` because it creates an internal feedback ticket. The guide keeps `staging_validated=false` and `deployment_triggered=false`.
...
The console also includes `market_signal_preview`, a read-only advisory rollup for internal Market Response preparation. It groups order lines, ordered quantity, feedback, production risk, and shipment risk across focus areas: adjustable desk frames, desk legs, lifting columns, education furniture, project furniture, and other products. Each focus row includes a `signal_score`, `primary_signal`, `review_label`, and Market page `route_query` so operators can open the relevant focus review from Portal Operations. This preview does not create tickets, change recommendations, notify customers/suppliers, or rank partners.
