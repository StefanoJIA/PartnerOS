...
Customer snapshots include `progress_steps` for Portal order tracking: confirmed, in production, ready to ship, shipped, and delivered. They also include `next_action_label`, `next_action_detail`, `tracking_summary`, and `customer_timeline` so the service Portal can show the next customer-visible step, data availability signals, and a combined order/production/shipment/resource/feedback timeline without inventing copy. These fields are display guidance only; planned dates remain non-guaranteed planning data.
...
Portal Operations also summarizes resource readiness with portal-visible count, customer-visible-but-unpublished count, hidden published count, status/category counts, and signed-download safety flags. It shows metadata only and must not expose storage keys, backend paths, or raw download tokens.
