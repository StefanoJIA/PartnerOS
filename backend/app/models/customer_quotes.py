...
class QuoteLearningRecord(Base, TimestampMixin, UserAuditMixin):
    ...
    outcome_source_type: Mapped[str] = mapped_column(String(32), nullable=False, default="quote", index=True)
    outcome_source_id: Mapped[uuid.UUID | None] = mapped_column(UUID(as_uuid=True), nullable=True)
