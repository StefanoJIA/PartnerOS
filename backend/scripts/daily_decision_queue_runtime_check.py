...
def main() -> int:
    ...
    try:
        ...
        checks = [
            ...
            ("decision brief present", bool(payload.decision_brief) and all("下一步" in line for line in payload.decision_brief)),
            ("partner rollup present", bool(payload.partner_rollup) and any(row.total >= 1 for row in payload.partner_rollup)),
            ("product rollup present", bool(payload.product_rollup) and any(row.affects_pilot >= 1 for row in payload.product_rollup)),
            ("category rollup present", bool(payload.category_rollup) and any(row.key == "market response" for row in payload.category_rollup)),
