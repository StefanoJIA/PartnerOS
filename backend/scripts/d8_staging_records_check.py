...
def _evidence_issues(records: list[Path]) -> list[str]:
    ...
    for path in records:
        ...
        if data.get("allow_local_http") is True:
            issues.append(f"{path.name}:local_rehearsal_allows_local_http")
        if backend_host in {"localhost", "127.0.0.1", "::1"}:
            issues.append(f"{path.name}:local_rehearsal_uses_local_backend")
